    ...
    def data_received(self, data):
        ...
        if len(mparse) > 1:
            if len(mparse) == 2:
                ...
                if len(m.split(',')) == 5:
                    self.msgs.append(m)
                    if (self.msginc >= MSGLONGINC):
                        self.msgslong.append(m)
                        self.msginc = 0
                    self.msginc += 1
                    print("{}".format(m))
            self.msgpart = ""
        if len(self.msgs) > MSGMAX:
            self.msgs.pop(0)
        if len(self.msgslong) > MSGLONGMAX:
            self.msgslong.pop(0)
...
async def lookback(websocket, ser):
    print(len(ser.msgslong))
    for msg in (ser.msgslong[i] for i in range(0,len(ser.msgslong),LOOKBACK_INTERVAL)):
        try:
